...
import {
  ...
  Trophy, Megaphone, User, Settings, GraduationCap, LogOut,
  Users, BarChart3, Shield, ChevronLeft, ChevronRight, Menu
} from "lucide-react";
      ...
      <div className="flex items-center gap-3 px-4 py-5 border-b border-sidebar-border">
        ...
        {!collapsed && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="overflow-hidden">
            ...
            <p className="text-xs text-sidebar-muted">{isAdminRoute ? "Admin Panel" : "Learning Platform"}</p>
          </motion.div>
        )}
      </div>
        ...
        {!collapsed && profile && (
          <div className="flex items-center gap-3 px-3 py-2">
            ...
            <div className="min-w-0">
              <p className="text-sm font-medium truncate text-sidebar-foreground">{profile.display_name}</p>
              <p className="text-xs text-sidebar-muted">{isAdmin ? "Admin" : "Student"}</p>
            </div>
          </div>
        )}
        <button
